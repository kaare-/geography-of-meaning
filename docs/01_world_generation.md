...
Fields exist (`erosion_damage`, `porosity`, `permeability`, `organic`). Surface and groundwater flow in `water.rs` now transport clay/organic sediment, raise `erosion_damage` along flow paths, and deposit coarse mineral/clay when inflow exceeds outflow. Cave formation and full landscape-as-memory are **planned**.
...
| `water.rs` | Rain, flow, infiltration, evaporation, groundwater, sediment transport & deposition |
