...
|----------|-------|------------------------------|------------|
| **Energy** | 0–1 | Passive drain via `tick_passive_drain(metabolism)` (default `0.007`); replenished by `ConsumeOrganic`; 3-tick grace before energy death | Action availability, survival |
...
| **Integrity** | 0–1 | Default 1.0; gradual loss from temperature stress (>0.85) and fatigue (>0.9); erosion damage in engine | Survival |
...
| Death when energy or integrity hits zero | `lifecycle.rs` (energy: after `ENERGY_DEPLETION_GRACE_TICKS` at zero) |
