...
`sim-core/src/memory/concepts.rs` — `ConceptNode` (prototype sensor vector, member node ids, strength), `ActiveConcept`, `activate_concepts()`. Sleep consolidation clusters ≥2 similar sensory nodes (cosine > 0.75) into concepts; loose sensory nodes merge into existing clusters during `consolidate_sleep`. Creature `concept_nodes` and `active_concepts` updated in `creature.rs` / `engine.rs`. `concepts_formed` per tick in `TickLogEntry`. Exported in `CreatureSnapshot`.
